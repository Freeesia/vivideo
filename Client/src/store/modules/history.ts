...
export default class History extends VuexModule {
  ...
  public watch(payload: { path: string; current: number }) {
    ...
    if (i === -1) {
      this.videos.push({ path, current, lastUpdate: new Date() });
    } else {
      this.videos.splice(i, 1, { path, current, lastUpdate: new Date() });
    }
  }
  ...
  public end(path: string) {
    const i = this.videos.findIndex(v => v.path === path);
    if (i === -1) {
      this.videos.push({ path, current: -1, lastUpdate: new Date() });
    } else {
      this.videos.splice(i, 1, { path, current: -1, lastUpdate: new Date() });
    }
  }
}
